...
def test_translate_single_tree(forest: RandomForest):
    ...
    assert forest.saved_forest_[0]  # saved_forest_[0] will be filled after translation
    assert all(forest.saved_forest_[i] == {} for i in range(1, forest.ntree))

    # numNodes = forest.saved_forest_[0]["children_right"].size
    # assert not any(forest.saved_forest_[0][key].size != numNodes for key in fr.saved_forest_[0].keys() )
...
def test_all_trees(forest: RandomForest):
    ...
    for i in range(forest.ntree):
        ...
        # assert not any(forest.saved_forest_[i][key].size != numNodes for key in forest.saved_forest_[i].keys())
        assert len(forest.saved_forest_[i]["feature"]) == num_nodes
